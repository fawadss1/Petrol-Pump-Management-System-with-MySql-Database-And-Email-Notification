from DB_Connection import db
from Narrator import Narrator
import datetime as T

y = T.datetime.now() - T.timedelta(days=1)
x_p = y.strftime("%d/%b/%Y")


print("*" + "~~" * 35 + "*")
Narrator("Welcome To Fawad Petrol Pump Management System")
print("*" + "~~" * 35 + "*")
Narrator("Please Note That This System can Support Only 2 Machines 1 Petrol and 1 Diesel")
print("*" + "~~" * 35 + "*")
Narrator("Please Provide Me Correct Information To Create Your Account Thank You")
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Name : ")
name = input().capitalize()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Email Address : ")
email = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Petrol Pump Name : ")
pump_name = input().capitalize()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Petrol Pump Registration Number : ")
reg_no = input().capitalize()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your UserName : ")
username = input().lower()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Password : ")
password = input().lower()
print("*" + "~~" * 35 + "*")
Narrator("Enter Current Petrol Price : ")
petrol_price = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Current Diesel Price : ")
diesel_price = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Petrol Reading 'A' : ")
petrol_A = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Petrol Reading 'B' : ")
petrol_B = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Diesel Reading 'A' : ")
diesel_A = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Diesel Reading 'B' : ")
diesel_B = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Petrol Stock : ")
petrol_stock = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Diesel Stock : ")
diesel_stock = input()
print("*" + "~~" * 35 + "*")
Narrator("Enter Your Current Month Balance : ")
current_mnth_balance = input()
print("*" + "~~" * 35 + "*")
Narrator("Your Account Has Been Created Successfully")
print("*" + "~~" * 35 + "*")

f = db.cursor()
f.execute("SHOW TABLES")
tbl = f.fetchall()
if ("users",) in tbl:
    pass
else:
    f.execute("CREATE TABLE `users` (name VARCHAR(15),email_addres VARCHAR(255),pump_name VARCHAR(255),reg_no VARCHAR(255),username VARCHAR(255),password VARCHAR(255))")
f.execute("INSERT INTO `users` (name,email_addres,pump_name,reg_no,username,password) VALUES ('" + name + "','" + email + "','" + pump_name + "','" + reg_no + "','" + username + "','" + password + "')")
db.commit()

f.execute("SHOW TABLES")
tbl = f.fetchall()
if ("day_summary",) in tbl:
    pass
else:
    f.execute("CREATE TABLE `day_summary` (date VARCHAR(15),ptrl_price VARCHAR(255),desl_price VARCHAR(255),ppra VARCHAR(255),pprb VARCHAR(255),ptrl_sld VARCHAR(255),ptrl_rs VARCHAR(255),ptrl_stck VARCHAR(255),dpra VARCHAR(255),dprb VARCHAR(255),desl_sld VARCHAR(255),desl_rs VARCHAR(255),dsil_stck VARCHAR(255),expanses VARCHAR(255),cashback VARCHAR(255),name VARCHAR(255),ttl_dy_rs VARCHAR(255), ttl_mnth_rs VARCHAR(255))")
f.execute("INSERT INTO `day_summary` (date,ptrl_price,desl_price,ppra,pprb,ptrl_stck,dpra,dprb,dsil_stck,ttl_mnth_rs) VALUES ('" + x_p + "','" + petrol_price + "','" + diesel_price + "','" + petrol_A + "','" + petrol_B + "','" + petrol_stock + "','" + diesel_A + "','" + diesel_B + "','" + diesel_stock + "','" + current_mnth_balance + "')")
db.commit()
